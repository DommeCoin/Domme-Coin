Dommecoin Core version *0.15.1* is now available from:

  <https://download.dommecoin.org/dommecoin-0.15.1/>
...
A number of changes to the way Dommecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 whips per kB (0.00001 DOM). This is to prepare for dropping the minimum transaction fee to 0.1 whips per kB (0.0001 DOM) in the next release.
