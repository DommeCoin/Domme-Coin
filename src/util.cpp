...
const char * const BITCOIN_CONF_FILENAME = "dommecoin.conf";
const char * const BITCOIN_PID_FILENAME = "dommecoind.pid";
...
#else
    const char* pszModule = "dommecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dommecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Dommecoin";
#else
    // Unix
    return pathRet / ".dommecoin";
#endif
#endif
}
